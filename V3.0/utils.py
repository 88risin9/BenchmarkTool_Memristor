...
def generate_random_matrix(rows: int, cols: int, low: float = 0.0, high: float = 1.0) -> np.ndarray:
    """
    Generate random conductance matrix
    生成随机电导矩阵
    :param rows: Number of rows
    :param cols: Number of columns
    :param low: Minimum value (normalized)
    :param high: Maximum value (normalized)
    :return: Random matrix
    """
    # Generate normalized values (0-1)
    return np.random.uniform(low=low, high=high, size=(rows, cols))
    # Note: These will be mapped to physical values (0-150μS) during calculations
